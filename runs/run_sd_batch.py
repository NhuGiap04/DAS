...
def _build_sd_cmd(args, prompt, run_output_dir):
    cmd = [
        args.python,
        str(args.sd_script),
        "--prompt",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run runs/single/sd.py over prompts from a .txt or .json file.",
    )
    parser.add_argument("--prompts_file", type=Path, required=True, help="Path to .txt or .json prompts file.")
    parser.add_argument("--sd_script", type=Path, default=Path("runs/single/sd.py"), help="Path to the single-prompt SD script.")
    parser.add_argument("--python", type=str, default=sys.executable, help="Python executable used to launch sd.py.")
    parser.add_argument("--output_dir", type=Path, default=Path("logs/sd_batch"))
