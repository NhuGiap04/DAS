...
def parse_args(model_label: str, default_script: Path, default_output_dir: Path):
    ...
    parser.add_argument("--save-final-artifacts", action="store_true", help="Save per-run final images, rewards JSON, and reward CSVs.")
    ...
    for local_idx, prompt in enumerate(selected_prompts, start=1):
        ...
        if proc.returncode == 0:
            ...
            if args.save_final_artifacts:
                ...
                final_images = list(run_output_dir.rglob("final_particle_*.png"))
                if final_images:
                    print(_c("  final images:", _Style.DIM), f"{len(final_images)} files")
        else:
            rows.append({
                "index": global_idx,
                "status": "FAIL",
